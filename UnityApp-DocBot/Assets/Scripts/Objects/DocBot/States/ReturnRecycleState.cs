...
namespace Objects.DocBot.States // PROPER HIERARCHY (Stores all of DocBot's states)
{
    ...
    public class ReturnRecycleState<TNm> : State<TNm> // TNm determines the datatype of the name (key)
    {

        private DocBotFSM fsm;
        
        public ReturnRecycleState(DocBotFSM fsm, TNm typeName, GenericState<TNm> stateManager) : base(stateManager, typeName) 
        ...
        public override void Update()
        {
            ...
            if (Vector3.Distance(fsm.transform.position, fsm.recyclingTransform.position) < 3) // if its nearby the recycling area
            {
                ...
                fsm.stateManager.ChangeState(DocBotFSM.DocBotTypes.RECYCLE_BOT);
            }

...
namespace Objects.DocBot.States // PROPER HIERARCHY (Stores all of DocBot's states)
{
    ...
    public class ReturnSupplyState : State<string> // TNm determines the datatype of the name (key)
    {

        private DocBotFSM fsm;
        
        public ReturnSupplyState(DocBotFSM fsm, string typeName, GenericStateManager<string> stateManager) : base(stateManager, typeName) 
        ...
        public override void Update()
        {
            ...
            if (Vector3.Distance(fsm.transform.position, fsm.resupplyTransform.position) < 3) // if its nearby 
            {
                ...
                fsm.stateManager.ChangeState("RESUPPLY_MATERIALS");
            }
        }

...
namespace FSM // PROPER HIERARCHY
{
    ...
    public class GenericState<TNm> // note that it's not implementing MonoBehavior because each 
...
// TNm means Type Name. Generic Datatype for whatever we want to define each state as for keys.
{
    ...
    private Dictionary<TNm, State<TNm>> states = new Dictionary<TNm, State<TNm>>();
    
    public State<TNm> CurrentState { get; set; }


    // #AddState - Add a state to the dictonary
    // @param - receives a key and value
    public void AddState(TNm key, State<TNm> value)
    {
    ...
    private State<TNm> GetState(TNm typeName)
    {
    ...
    public TNm GetCurrentStateName()
    {
    ...
    public void ChangeState(TNm typeName)
    {
        ...
        State<TNm> newState = GetState(typeName);
